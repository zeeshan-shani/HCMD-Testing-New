...
import { ListenIssueCommentCreated } from 'utils/wssConnection/Listeners/IssuesListeners';
...
export const RequestComments = ({ requestData, user }) => {
    const [addCommentsFlag, setAddComments] = useState(false);
    const textArea = useRef(null);

    useEffect(() => {
        ListenIssueCommentCreated(requestData);
    }, [requestData])
                            ...
                            {searchComments &&
                                <div className="comments-wrapper task-comments-wrapper mt-1" style={{ minHeight: addCommentsFlag ? 'auto' : '150px' }}>
                                    {!!searchComments.length &&
                                        searchComments
                                            .map((comment, index) => (
                                                <IssueCommentDetails user={user} key={index} comment={comment} />
                                            ))}
                                    {!searchComments.length && (<>
