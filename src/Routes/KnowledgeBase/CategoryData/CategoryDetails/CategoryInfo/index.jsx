...
export default function CategoryInfo() {
    ...
    const { data: subCategories = [], isFetching: loading, refetch } = useQuery({
        queryKey: ["/taskCategory/list", { ...newSearch }],
        queryFn: async () => {
            let payload = {};
            ...
            const data = await labelsubCategoryService.list({
                payload: { query: { labelId: activeCard?.id, ...payload }, }
            });
            if (data?.status === 1) {
                dispatch({ type: ISSUE_CONST.SET_CARD_SUBCATEGORIES, payload: subCategories });
                return data.data;
            }
            return [];
        },
    ...
    const onDeleteHandler = useCallback(async (item) =>
        TakeConfirmation({
            ...
            onDone: async () => {
                ...
                queryClient.setQueryData(["/taskCategory/list", { ...newSearch }],
                    (prev) => prev ?
                        prev.subCategories.filter((item) => {
