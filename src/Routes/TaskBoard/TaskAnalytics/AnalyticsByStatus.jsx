...
export default function AnalyticsByStatus({ analytics, range, setRange }) {
    const { user } = useSelector(state => state.user);

    return (
        <div className='task-status-analytics px-2'>
            <div className="dropdown mx-1">
                <button className="btn btn-sm dropdown-toggle text-capitalize text-color" id="userStatusDropdown" data-bs-toggle="dropdown">
                    {range?.label}
                </button>
                <ul className="dropdown-menu m-0" aria-labelledby="userStatusDropdown">
                    {timeRange.map((durr, index) => {
                        return (
                            <li key={index} className="dropdown-item text-capitalize" onClick={() => setRange(durr)}>
                                {durr.label}
                            </li>
                        )
                    })}
