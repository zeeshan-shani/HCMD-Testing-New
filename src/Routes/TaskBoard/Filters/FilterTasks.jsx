						...
						<div className={`p-1 m-0 navbar-nav w-100 justify-content-around`}>
							<div className="d-flex align-items-center mb-2 mb-lg-0">
								<div className="dropdown flex-100">
									<button className="btn btn-outline-default btn-sm dropdown-toggle text-truncate d-flex align-items-center border-0 transparent-bg" id="memberDropdown" data-bs-toggle="dropdown">
										<div id={`chat-${activeTaskChat?.id}`} className={`${classes.member} m-0`} title={name}>
											<img className="" src={getImageURL(image, '50x50')} alt="m" />
										</div>
									...
									<ul className="dropdown-menu m-0 position-absolute" aria-labelledby="memberDropdown">
										<li key={0} className="dropdown-item cursor-pointer pl-2 py-2" onClick={() => NewChatSelected(CONST.CHAT_TYPE.ALL_CHATS)}>
											<div>All Chats</div>
										</li>
										{chatList.map((chat) => {
											if (chat.type === CONST.CHAT_TYPE.GROUP) {
												...
												return (
													<li key={chat.id} className="dropdown-item cursor-pointer p-4_8" onClick={() => NewChatSelected(chat)}>
														<div id={`chat-${chat.id}`} className={`${classes.member}`}>
															<img src={getImageURL(image, '50x50')} alt="m" />
														</div>
														<div className="text-capitalize ml-1 text-truncate width-limit-8">{name}</div>
													</li>
												)
											}
											else if (chat.type === CONST.CHAT_TYPE.PRIVATE) {
												const findedUser = getPrivateChatUser(chat);
												// chat.chatusers.find(x => x.userId !== user.id)?.user ||
												// ((chat.users[0] === chat.users[1]) ? chat.chatusers[0].user : null);
												const name = findedUser?.name ? findedUser?.name : 'Disbled User';
												const profilePicture = findedUser?.profilePicture;
												return (
													<li key={chat.id} className="dropdown-item cursor-pointer p-4_8" onClick={() => NewChatSelected(chat)}>
														<div id={`chat-${chat.id}`} className={`${classes.member}`}>
															<img src={getImageURL(profilePicture, '50x50')} alt="m" />
														</div>
														<div className="text-capitalize ml-1 text-truncate width-limit-8">{name}</div>
													</li>)
											}
