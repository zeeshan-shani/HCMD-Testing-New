...
export default function UsersInfo() {
    ...
    let { data: usersStatusCount } = useQuery({
        ...
        queryFn: async () => {
            ...
            if (data?.status) {
                const statuses = data.data.flatMap((item, index) => ({ [item.profileStatus]: item.hasOwnProperty('count_profileStatus') && parseInt(item['count_profileStatus']) }));
    ...
    const gotoChat = useCallback(async (chat) => {
        ...
        const payload = await generatePayload({
            ...
            options: { "populate": ["lastMessage"], attributes: { exclude: ["users"] } },
            isCount: true
        })
        if (res?.status === 1) {
