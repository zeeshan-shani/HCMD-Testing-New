...
export default function Properties({ field, setField, setState }) {
        ...
        <div className='h-100'>
            <Card>
                ...
                <Card.Body className='overflow-scroll hide-horizonal-scroll' style={{ maxHeight: "90vh" }}>
                    {field && field?.id ?
                        <FieldProperties key={field?.id} nodeId={field.id} setField={setField} setState={setState} /> :
...
const FieldProperties = ({ nodeId, setField, setState }) => {
    ...
    const formData = useMemo(() => {
        ...
        const beforeSubmit = async (setError, data) => {
        ...
        const jsonForm = getJsonForm({ type: node.type, data: node.data, nodesOption });
        return (
            <FormGenerator
                className="m-0"
                dataFields={jsonForm}
