...
export default function ChatsContentSidebarList({
    ...
    useEffect(() => {
        ...
        ListenSingleChat({ activechatId: activeChat && activeChat.id, userId: user.id });
    }, [activeChat, user]);
                                ...
                                if (filterUsers === item.type || filterUsers === CONST.CHAT_TYPE.ALL_CHATS) {
                                    if (item.type === CONST.CHAT_TYPE.GROUP) {
                                        ...
                                        return (
                                            <li className={`contacts-item ${(item.id === activeChat?.id) ? 'active' : ''} ${totalUnread > 0 ? 'unread' : ''}`} key={item.id}>
                                                <div className="contacts-link" onClick={() => setUserHandler({ chat: item, activeChatId: activeChat?.id, userId: user.id, navigate })}>
                                                    <GroupItemContent name={name} image={image} item={item} user={user} myChatDetails={myChatDetails} />
                                                </div>
                                            </li>
                                        );
                                    }
                                        ...
                                        return (
                                            <li className={`contacts-item ${(item.id === activeChat?.id) ? 'active' : ''} ${totalUnread > 0 ? 'unread' : ''}`} key={item.id}>
                                                <div className="contacts-link" onClick={() => setUserHandler({ chat: item, activeChatId: activeChat?.id, userId: user.id, navigate })}>
                                                    <PrivateItemContent isSA={isSA} name={name} profilePicture={profilePicture} profileStatus={profileStatus} user={user} item={item} myChatDetails={myChatDetails} />
                                                </div>
                                            </li>
                                        );
                                    }
                                                ...
                                                <div className="contacts-link p-1 pt-3 position-relative">
                                                    <div className="contacts-content px-1">
                                                        ...
                                                        <div className="contacts-texts justify-content-start" onClick={() => { onClickMessage(item) }}>
                                                            {!item?.isDeleted ? <ResultMessage item={item} searchText={newGlobal} user={user} /> : <p className='font-weight-normal deleted-message text-color mb-0' style={{ fontSize: user?.fontSize }}>{CONST.TEMPLATE_MSG.DELETE}</p>}
                                                        </div>
                                                    </div>
                                                    {!item?.isDeleted && <SearchMessageDropdown item={item} user={user} setChatState={setChatState} chatData={chatData} activeChatId={activeChat?.id} />}
                                                </div>
                                            </li>);
                                    }
                                    else {
...
export const ResultMessage = ({ item, searchText, user }) => {
