...
export const ChatsContentSidebarHeader = ({
    ...
    const { globalSearch, hiddenChatNotiCount, hiddenCountTime, hiddenChats } = useSelector((state) => state.chat);
    ...
    const onFilterUsersHandler = useCallback((type, isHiddenCall) => {
        setChatSidebarState(prev => ({ ...prev, filterUsers: type }))
        // if (!isHiddenCall) setChatSidebarState(prev => ({ ...prev, hiddenChats: false }));
        if (!isHiddenCall) dispatch({ type: CHAT_CONST.SET_HIDDEN_CHATS, payload: false });
    }, [setChatSidebarState]);

    // onChange filter type to hidden 
    const onFilterHideChat = useCallback((type) => {
    ...
    const updateProfilePicture = useCallback(() => changeModel(CHAT_MODELS.PROFILE_PIC), []);

    // Is user clocked out
    const isClockOut = useMemo(() => user?.clockTime?.clockin?.length === user?.clockTime?.clockout?.length
        , [user?.clockTime?.clockin?.length, user?.clockTime?.clockout?.length]);

    // Function for clock out user
    const clockOutHandler = useCallback(() => SocketEmiter(SOCKET.REQUEST.CREATE_USER_LOG, { clockout: true }), []);

    // Function for change search call
    const onChangeSearch = useCallback((value, arrValue = []) => {
    ...
    const onGhostModeChange = useCallback(async () => {
        await updateUserAPI({ userId: user.id, isGhostActive: !user?.isGhostActive });
        SocketEmiter(!user?.isGhostActive ? 'join-chat-ghost-mode' : 'leave-chat-ghost-mode')
        navigate('/chats');
    ...
    useEffect(() => {
        if (hiddenChats && hiddenCountTime)
            dispatch({ type: CHAT_CONST.VERIFY_HIDDEN_CHAT_MENTION, userId: user.id });
    ...
    const onChangeTab = useCallback((e, val) => {
            ...
            <div className="sidebar-header sticky-top bg__chat-dark" style={{ padding: ".75rem .75rem  0 .75rem" }}>
                ...
                <div className="d-flex py-1 justify-content-between">
                    <div className="dropdown">
                        <button className="btn btn-outline-default dropdown-toggle text-capitalize custom-dropdown" id="chatFilterDropdown" data-bs-toggle="dropdown">
                            {hiddenChats ? "Hidden" : filterUsers}
                            {Boolean(hiddenChatNotiCount) && <span className="sidebar-notification" />}
                        </button>
                        <ul className="dropdown-menu m-0" aria-labelledby="chatFilterDropdown">
                            <li className="dropdown-item" onClick={() => onFilterUsersHandler(CONST.CHAT_TYPE.ALL_CHATS)}>Chats</li>
                            <li className="dropdown-item" onClick={() => onFilterUsersHandler(CONST.CHAT_TYPE.PRIVATE)}>Private</li>
                            <li className="dropdown-item" onClick={() => onFilterUsersHandler(CONST.CHAT_TYPE.GROUP)}>Groups</li>
                            <li className="dropdown-item" onClick={() => onFilterHideChat(CONST.CHAT_TYPE.ALL_CHATS)}>
                                Hidden {Boolean(hiddenChatNotiCount) &&
                    ...
                    <div className="input-group search-bar_chat-header w-100" style={{ maxWidth: "calc(100% - 96px)" }}>
                        <MultiSelectTextInput
                            setInputValue={setInputValue}
                            inputValue={inputValue}
                            setValue={(data) => {
                                if ((chatSidebarState.listType === ListType.PATIENT ||
                                    chatSidebarState.listType === ListType.CATEGORY) && data.length <= 1)
                                    return setValue(data)
                                else if (chatSidebarState.listType === ListType.CHAT) setValue(data)
                            }}
                            value={value}
                ...
                <div className={`badges ${"d-flex justify-content-end"}`} ref={advanceSearchRef}>
                    <Badge pill bg={advanceKeysLength ? "success" : "secondary"} text="light" className='p-1 px-2 mr-1 cursor-pointer'
                        onClick={() => setAdvanceSearch(prev => ({ ...prev, model: true }))}>
                        {`Advance Search`}
                    </Badge>
                    {!!advanceKeysLength &&
                        <Badge pill bg="secondary" text="light" className='p-1 px-2 mr-1 cursor-pointer'
