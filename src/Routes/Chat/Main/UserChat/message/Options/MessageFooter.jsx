        ...
        <div className='position-relative my-1'>
            <div className='message-footer d-flex align-items-center justify-content-between flex-wrap'>
                ...
                <div className="message-date text-capitalize message-info message-text d-flex" style={{ fontSize: user?.fontSize }}>
                    ...
                    <span className='mb-0'>{moment(item.createdAt).format("hh:mm A")}</span>
                    {item.sendBy === user.id &&
                        <span className='ml-1'>
                            {isBufferMsg ?
            ...
            {!hideReactions && !item.isDeleted && !!item?.messageEmojis?.length &&
                <div className='position-absolute reacted-div d-flex gap-5 px-1 emoji-font'
                    data-bs-toggle="collapse"
                    data-bs-target={`#messageReactions`}
                    aria-expanded="false"
                    aria-controls={`messageReactions`}
