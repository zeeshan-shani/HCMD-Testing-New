...
import { readAllMessages, SocketEmiter } from "utils/wssConnection/Socket";
...
export const ChatMessages = ({
    ...
    useDebounce(async () => {
        ...
        // console.log("checking next load", intersection.intersectionRatio);
    ...
    const checkIsUnread = useCallback((prevMsg, Msg, userId) => {
        try {
    ...
    const reactOnMessage = useCallback(async (body) => SocketEmiter("req-create-message-reaction", body), []);
                    ...
                    >
                        ...
                        {!!userChatState?.bufferMessages.length &&
                            <div className="message-day px-1" id="messageDay" key={-9} name={moment().format('MM/DD/YY')}>
                                ...
                                <BufferMessages messages={userChatState?.bufferMessages} user={user} SetUserChatState={SetUserChatState} messagesIds={messagesIds} />
                            </div>}
                        <InfiniteScroll
...
export const ScrolltoOrigin = async (qMessage, recursiveCount = 2000) => {
    try {
        if (messageRef[qMessage.id] &&
            ...
            const classes = messageRef[qMessage.id]?.current?.className;
            messageRef[qMessage.id].current.classList += " blink-quote-message ";
            setTimeout(() => messageRef[qMessage.id].current.classList = classes, 3000);
        } else {
