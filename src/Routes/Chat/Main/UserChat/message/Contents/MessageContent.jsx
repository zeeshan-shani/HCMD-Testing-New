...
export const MessageContent = ({
    ...
    const viewPatientInfo = useCallback(async (patient) => {
        ...
        const [string, id, name] = str.match(regex); // eslint-disable-line
        if (!id) return;
        toastPromise({
            ...
            <ErrorBoundary>
                <div className={`message-content position-relative ${!item?.isMessage ? 'task-msg-border' : ''} ${Boolean(item?.followupTaskMessageId) ? "followup-task" : ""} ${classes}`} style={{ fontSize: user?.fontSize }}>
                    {(!isPrivateChat || (getGhostAccess(user))) &&
                        <MessageHeader user={user} activeChat={activeChat} item={item} prevMsg={prevMsg} pTime={pTime} forceView={DisableOpt} setInspectUser={!ReadOnly ? setInspectUser : () => { }} />}
                    ...
                    {!DisableOpt && !ReadOnly &&
                        <QuotedMessage user={user} item={item} moveToOrigin={moveToOrigin} />}
    ...
    return (
        <ErrorBoundary>
            <div className={`message-content position-relative ${!item?.isMessage ? 'task-msg-border' : ''} ${classes}`} style={{ fontSize: user?.fontSize }}>
                ...
                <MessageCategory item={item} />
                <MessageFooter user={user} item={item} moveToOrigin={DisableOpt && moveToOrigin} isBufferMsg={isBufferMsg}
