...
import { sendMessage, SocketEmiter } from "utils/wssConnection/Socket";
...
export const ChatFooter = ({
    ...
    const { data: preTypedList, refetch: refetchPreMsg } = useQuery({
    ...
    const preTypedMessages = useMemo(() =>
        preTypedList ? preTypedList.map(item => ({ id: item.id, display: item.message })) : []
        , [preTypedList]);
    ...
    useEffect(() => {
        ...
        setMessageText(prev => {
            ...
            return savedData ? savedData : defaultInput;
        });
        setFooterState(prev => {
            ...
            // if (isPrivateChat) {
            //     const privateUser = activeChat.chatusers.find((item) => item.userId === privUsrId);
            //     privateUser && setAssignMem([privateUser])
            // }
    ...
    useEffect(() => {
        if (pastedFiles && !!pastedFiles.length) {
            const onChangeHandler = async () => {
                let selectedFiles = Object.keys(pastedFiles).map((item) => (
                    { file: pastedFiles[item], id: item + pastedFiles[item].lastModified }
                ));
                !!selectedFiles.length &&
                    setAttachmentFiles(prev => (prev && !!prev.length) ? [...prev, ...selectedFiles] : selectedFiles);
            }
            onChangeHandler();
        }
    }, [pastedFiles]);
    ...
    }, [activeChat.chatusers, activeChat.id, activeChat.type, assignMembers, clearFooterData, connected, footerState.isPrivateChat, footerState.privUsrId, messageText, quoteMessage, scrollDown, user.id, attachmentFiles?.length, onAttachmentsUpload, assignDesignation]);
    ...
    const sendMessageHandler = useCallback((msgType, e = undefined) => {
        ...
        if (messageText && messageText.message.trim() !== "") {
            ...
            inputRef?.current?.focus();
    }, [SetUserChatState, activeChat.chatusers, activeChat.id, activeChat.type, clearFooterData, messageText, quoteMessage, scrollDown, user.id, connected]);
    ...
    const sendEditMessage = useCallback((messageType, e = undefined) => {
        ...
        SocketEmiter(SOCKET.REQUEST.EDIT_CHAT_MESSAGE, body);
        setMessageText(defaultInput);
        SetUserChatState(prev => ({ ...prev, messageStatus: false, editMessage: null }))
    }, [SetUserChatState, connected, editMessage?.chatId, editMessage?.id, messageText, messages?.data?.rows]);
    ...
    const onSubmitAudio = useCallback(({ fileType, fileName, uploadedAudioUrl }) => {
        ...
        sendMessage(msgObject);
    }, [activeChat.chatusers, activeChat.id, activeChat.type, quoteMessage, user.id]);
    ...
    //     const handleClose = () => setFooterState(prev => ({ ...prev, contextmenu: null }));
    //     const onCopy = () => {
    ...
    //         const text = window.getSelection().toString();
    //         if (!text) return handleClose();
    //         handleClose();
    //         await toastPromise({
    //             func: async (resolve, reject) => {
    //                 try {
    //                     await preTypedMessageService.create({ payload: { message: text } });
    //                     refetchPreMsg();
    //                     resolve(1);
    //                 } catch (error) {
    //                     reject(0);
    //                 }
    //             }, loading: "Saving message", success: "Message saved", error: "Message couldn't save"
    //         })
    //     };
    //     return (
    //         <Menu
    //             open={footerState.contextmenu !== null}
    //             onClose={handleClose}
    //             anchorReference="anchorPosition"
    //             anchorPosition={
    //                 footerState.contextmenu !== null
    //                     ? { top: footerState.contextmenu.mouseY, left: footerState.contextmenu.mouseX }
    //                     : undefined
    //             }
    //         >
    //             <MenuItem onClick={onCopy}>Copy</MenuItem>
    //             <MenuItem onClick={onSaveSelection}>Save Selection</MenuItem>
    //         </Menu>
    //     )
    // }, [footerState.contextmenu, refetchPreMsg]);
    ...
    return (<>
        <div className="bg__chat-dark">
            ...
            {quoteMessage && !editMessage &&
                <QuoteMessage user={user} quoteMessage={quoteMessage} onCloseQuoteHandler={onCloseQuoteHandler} />}
            <div onClick={() => {
                if (!messageStatus) SetUserChatState(prev => ({ ...prev, messageStatus: true }))
            }}>
