...
export default function ChatsPage() {
	...
	const MainOutlet = useMemo(() => (
		<main className={`main ${(activeChat.id !== -1 || activePatientChat || activeCategoryChat) ? 'main-visible' : ''}`}>
			<Outlet context={[chatState, setChatState]} />
		</main>
	), [activeChat?.id, chatState, activePatientChat, activeCategoryChat]);
	...
	return (
		<ErrorBoundary>
			...
			{MainOutlet}
			{AppBar}
			{(taskName || name) && <div className="backdrop backdrop-visible" onClick={onBackdrop} />}
		</ErrorBoundary>
	);
};
