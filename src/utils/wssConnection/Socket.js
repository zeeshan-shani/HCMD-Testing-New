...
import {
	...
	ListenUserToOffline,
	ListenUserToOnline,
} from "utils/wssConnection/Listeners/chatListener";
...
import { getTaskAlerts, ListenTaskCommentMention } from "utils/wssConnection/Listeners/Tasklistener";
...
export const connectUserWithSocket = (userId) => {
	...
	ListenNewChat();
	ListenUserToOnline(userId);
	ListenUserToOffline(userId);
	ListenUserStatus(userId);
	ListenRemoveUserFromChat(userId);
	listenUpdateprofile(userId);
	listenUpdateGroup(userId);
};
...
export const MarkReadChat = (chatId) => {
	SocketEmiter(SOCKET.REQUEST.MARK_READ_CHAT, { chatId }, (data) => {
 ...
 */
export const receiveMessage = (activeChatId, userId, chatList) => {
	try {
...
/**
 * Function to request task comment update.
 * @param {object} data - The data for updating task comment.
 */
export const reqTaskCommentUpdate = (data) => SocketEmiter("manage-task-module:req-update-comment", data);
