...
import { clearBadge, generatePayload, setBadge, toastPromise } from "redux/common";
...
import { messageRef, ScrolltoOrigin } from "Routes/Chat/Main/UserChat/message/ChatMessages";
...
export const loadUserDashboardList = async (userId, filterData) => {
	try {
		if (cancelToken?.loadUserDashboardList) cancelToken?.loadUserDashboardList.cancel("Operation cancel due to new request.");
		...
		dispatch({ type: CHAT_CONST.LOAD_DASHBOARD_LIST_SUCCESS, payload: data.data });
...
export const loadUserChatList = async (payload) => {
	try {
		if (cancelToken?.loadUserChatList) cancelToken?.loadUserChatList.cancel("Operation cancel due to new request.");
		...
		if (!payload?.options?.pagination) {
			dispatch({ type: CHAT_CONST.LOAD_CHAT_LIST_SUCCESS, payload: data.data.rows });
...
export const getChatUsers = async (chatId) => {
	try {
		if (cancelToken?.getChatUsers) cancelToken?.getChatUsers.cancel("Operation cancel due to new request.");
...
export const setActiveChat = async (item, messageAt = null) => {
	try {
		...
		dispatch({ type: CHAT_CONST.SET_ACTIVE_CHAT_SUCCESS, payload: item, messageAt });
	} catch (error) {
...
export const getMessages = async ({ search = [], type = "", offset = 0, limit = CONST.MESSAGE_GET_LIMIT, filterMethod, taggedSearch }) => {
	try {
		const activeChat = getState().chat?.activeChat;
		const userId = getState().user?.user?.id;
		if (!taggedSearch && cancelToken?.getMessages) cancelToken?.getMessages.cancel("Operation cancel due to new request.");
		cancelToken.getMessages = axios.CancelToken.source();
		let config = { cancelToken: !taggedSearch && cancelToken?.getMessages.token, headers: {} };
		let payload = { search, type, filterMethod, chatId: activeChat.id, limit, offset };
		if (activeChat?.id !== -1) {
			const chatuser = activeChat?.chatusers?.find(usr => usr.userId === userId);
...
export const getMessages2 = async ({
	...
	try {
		...
		let config = { cancelToken: !taggedSearch && cancelToken?.getMessages.token, headers: {} };
		let payload = {
			search, type, filterMethod, chatId: activeChat.id, limit, offset, messageId, pagitionFlow, includeMessage
		};
		if (activeChat?.id !== -1) {
			const chatuser = activeChat?.chatusers?.find(usr => usr.userId === userId);
			if (chatuser && chatuser.hasOwnProperty('initialMessage'))
...
export const getUsersList = async (payload) => {
	try {
		if (cancelToken?.getUsersList) cancelToken?.getUsersList.cancel("Operation cancel due to new request.");
...
export const getResponseofThread = async (messageId, threadType = CONST.THREAD_TYPE.CHILD) => {
	try {
...
export const saveNotificationSettings = async ({
	...
	isImportantChat }) => {
	try {
		if (cancelToken?.saveNotificationSettings) cancelToken?.saveNotificationSettings.cancel("Operation cancel due to new request.");
		...
		const payload = { chatId, isRoutineNotificationMute, isEmergencyNotificationMute, isUrgentNotificationMute, isImportantChat };
...
export const addImportantMessage = async (message, type) => {
	try {
		...
		if (data.status === 1) dispatch({ type: CHAT_CONST.ADD_IMPORTANT_MESSAGE_SUCCESS, payload: data.data });
	} catch (error) {
		showError(error?.response?.data?.message);
		console.error(error);
	}
...
export const removeImportantMessage = async (message, type) => {
	try {
		...
		const data = await importantMessageService.update({ payload });
		if (data.status === 1) dispatch({ type: CHAT_CONST.REMOVE_IMPORTANT_MESSAGE_SUCCESS, payload: data.data });
	} catch (error) {
		showError(error?.response?.data?.message);
		console.error(error);
	}
...
export const addWatchMessage = async (message, type) => {
	try {
		...
		dispatch({ type: CHAT_CONST.ADD_WATCH_MESSAGE_SUCCESS, payload: data.data });
	} catch (error) {
		showError(error?.response?.data?.message);
		console.error(error);
	}
...
export const removeWatchMessage = async (payload, type, watchListId) => {
	try {
		...
		const data = await watchlistService.update({ payload });
		dispatch({ type: CHAT_CONST.REMOVE_WATCH_MESSAGE_SUCCESS, payload: data.data });
	} catch (error) {
		showError(error?.response?.data?.message);
		console.error(error);
	}
...
export const checkNotifications = async (dashboardList, chatList, userId) => {
	try {
		const getCounts = async () => {
			...
			let list = window.location.pathname.includes("dashboard") ? dashboardList : chatList;
			...
			chatCountArr = list?.map((chat) => {
				...
				const chatTotal = Number(
					...
					userData?.emergencyUnreadMessageCount + userData?.routineUnreadMessageCount + userData?.urgentUnreadMessageCount
				);
				return chatTotal;
			});
...
export const moveChatandQMessage = async ({ chatList, activeChat, user, qMessage, navigate }) => {
	try {
...
export const getPatientList = async (payload = {}) => {
	if (cancelToken?.getPatientList) cancelToken.getPatientList.cancel("Operation cancel due to new request.");
...
export const updatePatient = async (payload = {}) => {
	try {
		if (cancelToken?.updatePatient) cancelToken.updatePatient.cancel("Operation cancel due to new request.");
...
export const getMediaFiles = async (activeChatId, type = "media", search = "") => {
	const res = await getFilesData(activeChatId, type, search);
	if (res?.status) dispatch({ type: type === "media" ? CHAT_CONST.SET_MEDIA_FILES : CHAT_CONST.SET_DOCUMENT_FILES, payload: res?.data.rows });
};

export const getSendToUsers = (userId, chatType, chatusers = []) => {
	if (chatusers[0] && isNaN(chatusers[0])) {
		if (chatType === CONST.CHAT_TYPE.GROUP)
			return chatusers.filter((x) => x.userId !== userId).map((item) => item.userId);
