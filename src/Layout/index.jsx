...
export default function MainLayout() {
    ...
    const { activeChat, chatList, dashboardList, inspectUser, inspectPatient } = useSelector((state) => state.chat);
    ...
    useEffect(() => {
        ...
        listenChatActivities(activeChat.id, user.id);
    }, [activeChat?.id, user?.id]);

    useEffect(() => {
        receiveMessage(activeChat?.id, user && user?.id, !!chatList?.length ? chatList : dashboardList);
    }, [activeChat, activeChat?.id, chatList, dashboardList, user, navigate]);

    const setLocationChat = useCallback((val) => {
        if (val || (window.location.pathname.includes(CONST.APP_ROUTES.CHAT) && activeChat.id !== -1)) {
            ...
            history.listen(({ location, action }) => {
                if (action === "POP") {
                    if (window.matchMedia('(display-mode: standalone)').matches) {
                        ...
                        if (location.pathname === CONST.APP_ROUTES.TASK || location.pathname === "/tasks/list") {
                            setLocationChat(true);
                        }
                        else if (location.pathname.includes(CONST.APP_ROUTES.CHAT) && activeChat.id !== -1) {
                            setLocationChat();
                            // navigate(CONST.APP_ROUTES.CHAT);
                            if (taskName !== "") {
                                changeTask();
    ...
    return (
        <ErrorBoundary>
            <div className={tabClass}>
                <div className="main-layout prevent-overscroll-reload">
                    <div className="navigation navbar navbar-light bg-primary prevent-overscroll-reload">
                        {/* <!-- Logo Start --> */}
                        <Tooltip TransitionComponent={Zoom} title={CONST.APP_NAME} placement="right">
                            <Link className="d-none d-xl-block rounded p-1" to={CONST.APP_ROUTES.DASHBOARD}>
                        ...
                        <ul className="nav nav-minimal flex-row flex-grow-1 justify-content-around flex-xl-column justify-content-xl-center" id="mainNavTab" role="tablist">
                            {routesData.map((route) => {
                                route.isActive = pathname.includes(route.pathname) ? ' active' : '';
                                if (route.hasOwnProperty('access') && !route?.access) return null;
                                return (
                                    <li className={`${route.className} py-xl-3`} key={route.id} onClick={() => { route.onClick && route.onClick() }}>
                                        <Tooltip TransitionComponent={Zoom} title={route.title} placement="right">
                                            <Link id={route.id} className={route.linkClassName + route.isActive + " position-relative"} to={route.pathname}>
                                                {route.path === "tasks" && hasNotification && <span className="sidebar-notification blinking"></span>}
                                                {route.icon}
                                            </Link>
                                        </Tooltip>
                                    </li>)
                                ...
                                <li className="nav-item">
                                    <div className="toggle-theme-wrapper">
                                        <label className="toggle-theme" htmlFor="checkbox">
                                            <input type="checkbox" id="checkbox" onChange={toggleTheme} checked={user.themeMode === "dark"} />
            ...
            {/* <AppUpdate /> */}
            <UserInfoModal showModal={Boolean(inspectUser)} inspectUser={inspectUser} user={user} activeChat={activeChat} />
            {inspectPatient &&
                <PatientInfoModal showModal={Boolean(inspectPatient)} inspectPatient={inspectPatient} />}
...
function checkProvider(designations = []) {
    const restrict = designations.some(desg => CONST.PROVIDERS.includes(desg?.designation?.key));
    dispatch({ type: USER_CONST.UPDATE_FIELDS, payload: { isProvider: restrict } });
}
